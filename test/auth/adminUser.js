...
describe("管理員權限測試", function() { 
	...
	it("login", function(done){ 
		...
		.send({
			"email": userAdd.admin.signupEmail,
			"password": userAdd.admin.password
		})
	...
	it("user info", function(done){ 
		...
		.end(function(err,res){
			...
			expect(result.data._id).to.equal(user.admin._id.toString());
			done(); 
		}); 
	});
	...
	it("list name", function(done){ 
		...
		.set("x-requested-with","XMLHttpRequest")
		.query({id:user.admin._id.toString()})
		.end(function(err,res){
			expect(res.statusCode).to.equal(200);
			var result = JSON.parse(res.text);
			expect(result.status).to.equal("ok");
